...
$(function() {
    ...
    // WebUploader实例
    uploader;
    function craeteUploader(uploader) {
	...
	uploader = WebUploader.create({
	    ...
	    auto : true,
	    // swf文件路径
	    swf : BASE + '/thirdparty/webuploader015/Uploader.swf',
	    // 文件接收服务端。
	    server : BASE + '/admin/fileupload.php',
	    // 选择文件的按钮。可选。
	...
	uploader.on('uploadSuccess', function(file, response) {
	    ...
	    $('#' + file.id).append(
		    '<input type="hidden" name="imgPath" value="' + f.filePath
			    + '">');
	    $('#' + file.id).append(
		    '<input type="hidden" name="imgId" value="' + f.id + '">');
	});
	...
	uploader = craeteUploader(uploader);
