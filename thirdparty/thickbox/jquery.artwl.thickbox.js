    ...
    $.extend({
        artwl_bind: function (options) {
            ...
            var cssCode = 'html, body, h1, h2, h3, h4, h5{margin: 0px;padding: 0px;}\
                            ...
                            #artwl_showbox{padding: 10px;background: #FFF;border-radius: 5px;margin: 20px;min-width:100px;min-height:50px;}\
                            #artwl_title{position: relative;height: 30px;border-bottom: 1px solid #999;}\
                            #artwl_close{position: absolute;cursor: pointer;outline: none;top: 0;right: 0;z-index: 4;width: 42px;height: 42px;overflow: hidden;background-image: url(/thirdparty/thickbox/feedback-close.png);_background: none;}\
            ...
            $(window).resize(function(){
            	var height = $("#artwl_boxcontain").height();
                var width = $("#artwl_boxcontain").width();
            	$("#artwl_boxcontain").css("top", ($(window).height() - height) / 2).css("left", ($(window).width() - width) / 2);
                if ($.browser.msie && $.browser.version.substr(0, 1) < 7) {
                    width = $(window).width() > 600 ? 600 : $(window).width() - 40;
                    $("#artwl_boxcontain").css("width", width + "px").css("top", ($(window).height() - height) / 2).css("left", ($(window).width() - width) / 2).show();
                    $("#artwl_mask").css("width", $(window).width() + "px").css("height", $(window).height() + "px").css("background", "#888");
